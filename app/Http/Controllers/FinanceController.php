...
//            return $total_penagihan;
        ...
        return view('finance.report_project', compact('years', 'site_types', 'project_data','date_filter_decode', 'date_filter', 'check_ignore_filter'));
    }
...
//        return $budget_plan_ids;
        ...
        return view('finance.report_budget_dept_detail', compact('tahun', 'budget_plan_line_datas','budget_plan_line_departments'));
    }
...
//                                ((float)($data_check_budget_data->amount + $nilai_pengajuan)) / ((float)$data_check_budget_data->amount) * 100
    ...
    private function _get_site_customer(){
        return DB::table('res_partner')
            ->whereNotIn('id', [3])
            ->select('id', 'name')
            ->pluck('name', 'id');
    }
